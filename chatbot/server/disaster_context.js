...
function formatProtocolForPrompt(protokoll) {
  ...
  const formatEntry = (entry) => {
    ...
    // Information wird NICHT gekürzt - vollständiger Inhalt ist relevant
    const info = entry.information || entry.info || "";

    // Extrahiere befüllte Maßnahmen
    let massnahmenText = "";
    if (Array.isArray(entry.massnahmen)) {
      const befuellteMassnahmen = entry.massnahmen
        .filter(m => m && typeof m.massnahme === "string" && m.massnahme.trim())
        .map(m => m.massnahme.trim());
      if (befuellteMassnahmen.length > 0) {
        massnahmenText = ` | Maßnahmen: ${befuellteMassnahmen.join("; ")}`;
      }
    }
